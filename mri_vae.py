...
skull stripping Kleesiek, J 2016
    ...
    #do max pooling
    pool2 = tf.nn.max_pool3d(prev_layer,ksize = [1,2,2,2,1],
                             strides = [1,2,2,2,1],
                            padding= 'SAME')
...
def VAE_loss(x_ph,x_mu,z_mu,z_sigma,l,global_step,  learning_rate_initial = 0.01,learning_decay = 0.96):
    ...
    IAF_loss = l
    cost = tf.reduce_mean(reconstruct_loss + latent_loss + IAF_loss)
...
z_mu,z_sigma,h = inference_network(x_ph,latent_dimension = latent_dimension)
...
cost,optimiser = VAE_loss(x_ph,mu_x,z_mu,z_sigma,l_1,global_step)
...
with tf.Session() as session:
    ...
    for epoch in range(n_epoch):
        ...
        saver.save(session,'mri_vae_model/my-model.ckpt',write_meta_graph=True,global_step = epoch)
