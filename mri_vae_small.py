...
def _3D_deconv_layer(name, kernel_shape, biases_shape,prev_layer, activation_fn = tf.nn.elu,
    ...
    with tf.variable_scope(name) as scope:
        ...
        tf.add_to_collection('deconv_bias',biases)
        
        #get the output size of the convolution
...
def _3D_conv_layer(name, kernel_shape, biases_shape,prev_layer, activation_fn = tf.nn.elu,
    ...
    with tf.variable_scope(name) as scope:
        ...
        biases = _bias_variable('biases',biases_shape)
        tf.add_to_collection('conv_bias',biases)
        #apply weights and biases
...
def generative_network(z):
    ...
    biases_shape = [out_filter]
    prev_layer = _3D_deconv_layer('deconv4',kernel_shape,biases_shape,prev_layer)
    in_filter = out_filter
...
x_ph = tf.placeholder(tf.float32,[None, x_shape[0],x_shape[1],x_shape[2],1],name = 'input')
...
image = read_and_decode_single_example("T1_BL_scalecropped_train.tfrecords")
...
image_test = read_and_decode_single_example("T1_BL_scalecropped_test.tfrecords")
# groups examples into batches randomly
...
with tf.Session() as session:
    ...
    for epoch in range(n_epoch):
        ...
        saver.save(session,'mri_vae_model/my-model-small.ckpt',write_meta_graph=True,global_step = epoch)

    #lets obtain the weights
    conv_weights,deconv_weights,conv_bias,deconv_bias = session.run([tf.get_collection('conv_weights'),tf.get_collection('deconv_weights'),tf.get_collection('conv_bias'),tf.get_collection('deconv_bias')])

    #save weights and biases to file
    for i in range(len(conv_weights)):
        name = 'conv_weights' + str(i)
        np.save(name,conv_weights[i])
        name = 'conv_bias' + str(i)
        np.save(name,conv_bias[i])

    for i in range(len(deconv_weights)):
        name = 'deconv_weights' + str(i)
        np.save(name,deconv_weights[i])
        name = 'deconv_bias' + str(i)
        np.save(name,deconv_bias[i])
    
    #see if the reconstruction is any good
    x_test= session.run([images_batch_test])[0]
    x_test = np.reshape(x_test,(batch_size_test,x_shape[0],x_shape[1],x_shape[2],1))
